...
results->bestfvalue=dvector(1,params->node_num);
results->bestx=ivector(1,params->node_num);
results->bestmat=imatrix(1,params->node_num,1,params->node_num);
results->indicator=ivector(0,params->grid);
...
for(Repeat=1; Repeat<=1; Repeat++){
   ...
   // TODO what are accept_loc and total_loc?
   totalweight = 0.0;
