...
void copyParamData(simParams* params, double *refden, int *states, char *data){
  ...
  params->state=ivector(0,params->node_num-1);
  params->refden=dvector(0,params->grid-1);
  params->datax=imatrix(0,params->data_num-1,0,params->node_num-1);

  for(int i=0; i<rows; i++){
    for(int j=0; j<cols; j++){
      /*printf(" %d ", result[i][j]);*/
      params->datax[i][j] = result[i][j];
    }
  }
  
  /*printf("\n\nStates: \n");*/
  for(int i=0; i<cols; i++) {
    params->state[i]=states[i];
    /*printf(" %d ", states[i]);*/
    // Number of results->states in each column
  }

  /*printf("\n\nRefden: \n");*/
  for(int i=0; i<params->grid-1; i++){
...
simResults* pyInitSimResults(simParams* params){
  ...
  ret->bestfvalue=dvector(0,params->node_num-1);
  ret->bestx=ivector(0,params->node_num-1);
  ret->bestmat=imatrix(0,params->node_num-1,0,params->node_num-1);
  ret->indicator=ivector(0,params->grid-1);
...
void freeSimResults(simParams* params, simResults* results){
  ...
  free_dvector(results->bestfvalue,0,params->node_num-1);
  free_ivector(results->bestx,0,params->node_num-1);
  free_imatrix(results->bestmat,0,params->node_num-1,0,params->node_num-1);
  free_ivector(results->indicator,0,params->grid-1);
...
int freeSimParams(simParams *params){
  ...
  free_dvector(params->refden,0,params->grid-1);
  free_ivector(params->state,0,params->node_num-1);
  free_imatrix(params->datax,0,params->data_num-1,0,params->node_num-1);
  free(params);
  return 0;
}
